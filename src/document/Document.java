...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (char letter  : word.toCharArray()) {
			if (isVowel(letter) && word.length() == 1) {
				syllables++;
			} else if (isVowel(letter) && !isVowel(previousLetter)){
				if (letter != 'e') {
					syllables++;
				} else if (i < word.length() - 1 || syllables == 0){
					syllables++;
				}
			}
	...
	private boolean isVowel(char letter)
	{
		...
		return vowels.indexOf(Character.toLowerCase(letter)) == -1 ? false : true;
	}
